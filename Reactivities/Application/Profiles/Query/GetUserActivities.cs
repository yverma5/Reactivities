...
public class GetUserActivities
{
    ...
    : IRequestHandler<Query, Result<List<UserActivityDto>>>
    {
        ...
        CancellationToken cancellationToken)
        {
            ...
            {
                "past" => query.Where(a => a.Date <= today && a.Attendes.Any(x => x.UserId == request.UserId)),
                "hosting" => query.Where(a => a.Attendes.Any(x => x.IsHost
                && x.UserId == request.UserId)),
                _ => query.Where(a => a.Date >= today
                && a.Attendes.Any(x => x.UserId == request.UserId))
            };
